...
export type ShareSticker = Omit<FlightShareSticker, "id"> & { id: string };
...
export type ShareStickerControl = ToggleControl | TextControl | ChoiceControl;
...
type Props = {
  ...
  customSticker: ShareSticker | null;
  customControls: ShareStickerControl[];
  readyShowBackground: boolean;
  customShowBackground: boolean;
  onReadyShowBackgroundChange: (checked: boolean) => void;
...
function ChoiceOption({ control }: { control: ChoiceControl }) {
  return (
    <div>
      <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">{control.label}</p>
...
function CustomControl({ control }: { control: ShareStickerControl }) {
  ...
  if (control.kind === "choice") return <ChoiceOption control={control} />;
...
export function ShareStickersModal({
  ...
  customControls,
  readyShowBackground,
  customShowBackground,
  onReadyShowBackgroundChange,
            ...
            <div className="grid min-h-full gap-5 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
              ...
              <aside className="rounded-3xl border border-slate-800 bg-slate-900/50 p-4">
                ...
                <div className="mt-5">
                  ...
                  <ToggleOption
                    checked={mode === "ready" ? readyShowBackground : customShowBackground}
                    label="Mostrar fundo"
                    onChange={(checked) => {
                      if (mode === "ready") onReadyShowBackgroundChange(checked);
                      else onCustomShowBackgroundChange(checked);
                    }}
                  />
                </div>
                ...
                {mode === "ready" ? (
                  ...
                  <div className="mt-5 space-y-4">
                    {customControls.map((control) => (
                      <CustomControl key={control.id} control={control} />
                    ))}
