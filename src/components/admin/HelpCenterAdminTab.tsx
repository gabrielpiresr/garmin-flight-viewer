...
import {
  createHelpArticle,
  createHelpSection,
  createHelpSubsection,
  deleteHelpArticle,
  deleteHelpSection,
  deleteHelpSubsection,
  listHelpCatalog,
  updateHelpArticle,
  updateHelpSection,
...
export function HelpCenterAdminTab({ audience = "student" }: HelpCenterAdminTabProps) {
  ...
  function resetSubsectionForm(sectionId = selectedSectionId) {
    ...
    setSubsectionForm({ ...emptySubsectionForm, sectionId, order: String(subsectionsForSelected.length + 1) });
  ...
  function openArticleEdit(article: HelpArticle) {
  ...
  async function handleUploadMedia(file: File): Promise<ManeuverMediaUpload | null> {
    ...
    return result.data as HelpMediaUpload | null;
  }
            ...
            {sectionEditorOpen ? (
              <section className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setSectionEditorOpen(false)} className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-400 hover:bg-slate-800">Fechar</button>
                </div>
                <div className="grid gap-3 md:grid-cols-[1fr_7rem_10rem]">
                  <input value={sectionForm.title} onChange={(event) => setSectionForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Título da seção" className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500" />
                  <input value={sectionForm.order} onChange={(event) => setSectionForm((prev) => ({ ...prev, order: event.target.value }))} placeholder="Ordem" className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500" />
                  ...
                  <textarea value={sectionForm.description} onChange={(event) => setSectionForm((prev) => ({ ...prev, description: event.target.value }))} placeholder="Descrição opcional" rows={2} className="w-full resize-y rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500 md:col-span-3" />
                </div>
                <button type="button" disabled={saving} onClick={() => void handleSaveSection()} className="mt-3 rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-500 disabled:opacity-60">
                  {saving ? "Salvando..." : editingSectionId ? "Salvar seção" : "Criar seção"}
                </button>
              </section>
            ) : null}
            ...
            {subsectionEditorOpen && !hideSubsections ? (
              <section className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setSubsectionEditorOpen(false)} className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-400 hover:bg-slate-800">Fechar</button>
                </div>
                <div className="grid gap-3 md:grid-cols-[1fr_7rem_10rem]">
                  <input value={subsectionForm.title} onChange={(event) => setSubsectionForm((prev) => ({ ...prev, title: event.target.value }))} placeholder="Título da subseção" className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500" />
                  <input value={subsectionForm.order} onChange={(event) => setSubsectionForm((prev) => ({ ...prev, order: event.target.value }))} placeholder="Ordem" className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-500" />
                  <label className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-slate-300">
                ...
                <button type="button" disabled={saving || !selectedSectionId} onClick={() => void handleSaveSubsection()} className="mt-3 rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-500 disabled:opacity-60">
                  {saving ? "Salvando..." : editingSubsectionId ? "Salvar subseção" : "Criar subseção"}
                </button>
              </section>
            ) : null}
            ...
            {articleEditorOpen ? (
              <section className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-4">
                <div className="mb-4 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setArticleEditorOpen(false)} className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-400 hover:bg-slate-800">Voltar</button>
                </div>
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <label className="space-y-1 text-xs font-medium text-slate-400">
                    <span>Tags</span>
                    <input value={articleForm.tags} onChange={(event) => setArticleForm((prev) => ({ ...prev, tags: event.target.value }))} placeholder="Separadas por vírgula" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm font-normal text-slate-100 outline-none focus:border-cyan-500" />
                  </label>
                  <label className="space-y-1 text-xs font-medium text-slate-400">
                    <span>Ordem</span>
                    <input value={articleForm.order} onChange={(event) => setArticleForm((prev) => ({ ...prev, order: event.target.value }))} placeholder="Ordem" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm font-normal text-slate-100 outline-none focus:border-cyan-500" />
                  </label>
                ...
                <div className="mt-4">
                  <ManeuverRichTextEditor
                    value={articleForm.contentJson as ManeuverRichContent}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button type="button" disabled={saving} onClick={() => void handleSaveArticle()} className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-500 disabled:opacity-60">
                    {saving ? "Salvando..." : editingArticleId ? "Salvar artigo" : "Criar artigo"}
                  </button>
                  <button type="button" onClick={() => setArticleEditorOpen(false)} className="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-400 hover:bg-slate-800">Cancelar</button>
                </div>
              </section>
            ) : null}
              ...
              <div className={`grid gap-4 ${hideSubsections ? "" : "xl:grid-cols-[18rem_1fr]"}`}>
                ...
                <section className="rounded-2xl border border-slate-700/60 bg-slate-900/50 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    ...
                    <button type="button" onClick={() => resetSubsectionForm(selectedSectionId)} disabled={!selectedSectionId} className="rounded border border-slate-700 px-2 py-1 text-xs text-slate-300 hover:bg-slate-800 disabled:opacity-50">Nova</button>
                  </div>
                  <div className="space-y-2">
                    {subsectionsForSelected.length ? subsectionsForSelected.map((subsection) => (
                      <div key={subsection.id} className="rounded-lg border border-slate-700/60 bg-slate-950/30 p-3">
                        ...
                        <p className="text-[11px] text-slate-500">{subsection.isPublished ? "Publicado" : "Rascunho"}</p>
                        <div className="mt-2 flex gap-2">
                          ...
                          <button type="button" onClick={() => void handleDeleteSubsection(subsection)} className="rounded border border-red-700/40 px-2 py-1 text-xs text-red-300 hover:bg-red-500/10">Apagar</button>
                        </div>
                      </div>
                ...
                <section className="rounded-2xl border border-slate-700/60 bg-slate-900/50 p-4">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wider text-cyan-400/80">Artigos</p>
