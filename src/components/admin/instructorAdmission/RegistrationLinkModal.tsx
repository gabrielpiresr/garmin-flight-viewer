...
export function RegistrationLinkModal({
  ...
  const registrationUrl = token
    ? `${window.location.origin}/admissao-instrutor?token=${token}`
    : null;
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-slate-700/60 bg-[var(--panel)] shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-100">Link de registro</h2>
        ...
        <div className="space-y-4 p-5">
          <p className="text-xs text-slate-400">
            ...
            <span className="font-medium text-slate-200">{candidate.name}</span> preencher o formulário de
            candidatura.
          </p>
          {candidate.formFilledAt && (
            <div className="rounded-lg bg-emerald-900/20 px-3 py-2 text-xs text-emerald-400">
              ✓ Formulário já preenchido em{" "}
          ...
          {registrationUrl ? (
            <div className="space-y-2">
              <div className="flex gap-2">
                <input readOnly value={registrationUrl} className={inputCls} />
                <button
                  type="button"
                  onClick={copyLink}
            ...
            >
              {generating ? "Gerando..." : "Gerar link de registro"}
            </button>
          )}
        </div>
