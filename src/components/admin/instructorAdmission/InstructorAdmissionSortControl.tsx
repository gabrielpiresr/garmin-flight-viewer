import {
  INSTRUCTOR_ADMISSION_SORT_OPTIONS,
...
type Props = {
  ...
  sortAsc: boolean;
  onSortChange: (key: InstructorAdmissionSortKey, asc?: boolean) => void;
};

export function InstructorAdmissionSortControl({ sortKey, sortAsc, onSortChange }: Props) {
  const currentLabel = INSTRUCTOR_ADMISSION_SORT_OPTIONS.find((o) => o.value === sortKey)?.label ?? "Nome";
    ...
    <div className="flex items-center gap-1">
      <details className="group relative">
        <summary className={`${btnCls} min-w-0 cursor-pointer list-none [&::-webkit-details-marker]:hidden`}>
          ...
          <span className="max-w-[110px] truncate text-slate-200">{currentLabel}</span>
        ...
        <div className="absolute right-0 z-20 mt-1 min-w-[180px] overflow-hidden rounded-lg border border-slate-700 bg-[var(--panel)] p-1 shadow-xl">
          {INSTRUCTOR_ADMISSION_SORT_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              type="button"
              onClick={(e) => {
