...
function exportCsv(rows: AircraftFueling[]) {
  ...
  downloadBlob([headers.map(csvEscape).join(","), ...lines].join("\n"), "text/csv;charset=utf-8", "abastecimentos.csv");
}
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-4">
          <form onSubmit={handleSubmit} className="max-h-[92vh] w-full overflow-y-auto rounded-t-2xl border border-slate-800 bg-slate-950 p-4 shadow-2xl sm:mx-auto sm:max-w-3xl sm:rounded-2xl sm:p-5">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-100">Novo abastecimento</h3>
              <button type="button" onClick={() => setModalOpen(false)} className="rounded-lg border border-slate-700 px-3 py-1.5 text-sm text-slate-300 hover:bg-slate-800">Fechar</button>
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => setModalOpen(false)} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-800">Cancelar</button>
              <button disabled={saving} type="submit" className="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-emerald-400 disabled:cursor-wait disabled:opacity-60">{saving ? "Salvando..." : "Lançar abastecimento"}</button>
            </div>
          </form>
        </div>
      ) : null}
