  ...
  if (!configured) {
    return (
      <div className="rounded-xl border border-amber-500/30 bg-amber-950/20 px-4 py-3 text-xs text-amber-100/90">
        Defina <code className="text-amber-200">VITE_SUPABASE_URL</code> e{" "}
        <code className="text-amber-200">VITE_SUPABASE_ANON_KEY</code> no <code>.env.local</code> para ativar login e
        nuvem.
      </div>
    );
  }
