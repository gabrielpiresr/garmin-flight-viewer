...
function alreadySynced(metadata, desired = {}) {
...
function pickClassroom(classrooms) {
  const matches = (Array.isArray(classrooms) ? classrooms : []).filter(
    (row) => nameMatchesNaproa360(row?.course_name) || nameMatchesNaproa360(row?.name),
  );
  return matches.find((row) => row?.master === true) || matches[0] || null;
}
...
async function upsertMemberkitUser({ fullName, email, status, expiresAt }, env = process.env) {
