const TILE_HOST = "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile";
...
function parseTileIndex(value, label) {
  ...
  if (i < 0) throw new Error(`${label} invalido`);
  return i;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (z > 19) throw new Error("z invalido");
    const n = 2 ** z;
    if (x >= n || y >= n) throw new Error("tile fora do zoom");

    const response = await fetch(`${TILE_HOST}/${z}/${y}/${x}`);
    if (!response.ok) {
      res.status(response.status === 404 ? 404 : 502).json({ error: "Falha ao obter imagem de satelite." });
      return;
    }
