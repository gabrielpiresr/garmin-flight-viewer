...
const functionId = process.env.LASTLINK_WEBHOOK_FUNCTION_ID || "lastlink-webhook";
...
const endpoint = process.env.APPWRITE_ENDPOINT || envFile.VITE_APPWRITE_ENDPOINT || "https://sfo.cloud.appwrite.io/v1";
const projectId = process.env.APPWRITE_PROJECT_ID || envFile.VITE_APPWRITE_PROJECT_ID || "6a01ac8a0009fbf94f05";
const apiKey = process.env.APPWRITE_API_KEY || envFile.APPWRITE_API_KEY;
...
const studentTracksCollectionId =
  ...
  envFile.APPWRITE_STUDENT_TRACKS_COLLECTION_ID ||
  envFile.VITE_APPWRITE_STUDENT_TRACKS_COL_ID ||
  "student_training_tracks";
const frcMembershipsCollectionId =
  process.env.APPWRITE_FLIGHT_REVIEW_CLUB_MEMBERSHIPS_COLLECTION_ID ||
  envFile.APPWRITE_FLIGHT_REVIEW_CLUB_MEMBERSHIPS_COLLECTION_ID ||
  envFile.VITE_APPWRITE_FLIGHT_REVIEW_CLUB_MEMBERSHIPS_COL_ID ||
  "flight_review_club_memberships";
const frcTasksCollectionId =
  process.env.APPWRITE_FLIGHT_REVIEW_CLUB_TASKS_COLLECTION_ID ||
  envFile.APPWRITE_FLIGHT_REVIEW_CLUB_TASKS_COLLECTION_ID ||
  envFile.VITE_APPWRITE_FLIGHT_REVIEW_CLUB_TASKS_COL_ID ||
...
const memberkitApiKey = process.env.MEMBERKIT_API_KEY || envFile.MEMBERKIT_API_KEY || "";
const memberkitMembershipLevelId =
  process.env.MEMBERKIT_NAPROA360_MEMBERSHIP_LEVEL_ID || envFile.MEMBERKIT_NAPROA360_MEMBERSHIP_LEVEL_ID || "";
const memberkitClassroomId =
  process.env.MEMBERKIT_NAPROA360_CLASSROOM_ID || envFile.MEMBERKIT_NAPROA360_CLASSROOM_ID || "";
...
const deployment = await functions.createDeployment({
  functionId,
  code: InputFile.fromBuffer(archive, "lastlink-webhook.tar.gz"),
  activate: true,
...
for (let attempt = 0; attempt < 90; attempt += 1) {
  ...
  if (current.status === "ready") {
    console.log("Deployment LastLink webhook pronto e ativo.");
    break;
  }
