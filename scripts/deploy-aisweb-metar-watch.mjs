/**
 * Deploy the lightweight AISWEB METAR watch cron function.
 *
 * Reuses the admin-users package (same source tree) with entrypoint
 * src/metarWatchCron.js and schedule every 15 minutes.
 *
 * Usage:
 *   node scripts/deploy-aisweb-metar-watch.mjs
 */
...
const functionId = process.env.AISWEB_METAR_WATCH_FUNCTION_ID || "aisweb-metar-watch";
const schedule = process.env.AISWEB_METAR_WATCH_SCHEDULE || "*/15 * * * *";
const timeout = Number(process.env.AISWEB_METAR_WATCH_TIMEOUT || 60);
...
async function ensureFunction(functions) {
  const params = {
    ...
    enabled: true,
    logging: true,
    entrypoint: "src/metarWatchCron.js",
...
async function ensureArchive(env) {
  if (fs.existsSync(archivePath) && process.env.AISWEB_METAR_WATCH_REUSE_ARCHIVE === "1") {
...
async function copyVariablesFromAdminUsers(functions, fileEnv, apiKey) {
  ...
  for (const variable of sourceVars) {
    ...
    // Secret vars às vezes voltam vazias no list — repor pelas fontes locais.
    if ((value == null || value === "") && variable.key === "APPWRITE_API_KEY") value = apiKey;
    if ((value == null || value === "") && variable.key === "AISWEB_API_KEY") {
      value = process.env.AISWEB_API_KEY || fileEnv.AISWEB_API_KEY || "";
    }
...
async function main() {
  ...
  const code = InputFile.fromBuffer(buffer, "aisweb-metar-watch.tar.gz");
