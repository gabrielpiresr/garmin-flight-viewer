...
async function setupSections() {
  console.log("\n[1/4] maneuver_sections...");
  const col = await ensureCollection("maneuver_sections");
  const id = col.$id;
  ...
  await idx(id, "man_sec_order_idx", ["order"]);
  await idx(id, "man_sec_pub_order_idx", ["is_published", "order"], ["ASC", "ASC"]);
  return id;
}

async function setupSubsections() {
  console.log("\n[2/4] maneuver_subsections...");
  const col = await ensureCollection("maneuver_subsections");
  const id = col.$id;
  ...
  await attr(() => db.createIntegerAttribute(DATABASE_ID, id, "order", true), "order");
  await attr(() => db.createBooleanAttribute(DATABASE_ID, id, "is_published", true), "is_published");
  await idx(id, "man_sub_section_idx", ["section_id"]);
  await idx(id, "man_sub_order_idx", ["order"]);
  await idx(id, "man_sub_sec_order_idx", ["section_id", "order"], ["ASC", "ASC"]);
  return id;
}

async function setupArticles() {
  console.log("\n[3/4] maneuver_articles...");
  const col = await ensureCollection("maneuver_articles");
  const id = col.$id;
  await attr(() => db.createStringAttribute(DATABASE_ID, id, "section_id", 64, true), "section_id");
  ...
  await attr(() => db.createIntegerAttribute(DATABASE_ID, id, "order", true), "order");
  await attr(() => db.createIntegerAttribute(DATABASE_ID, id, "source_page_start", false), "source_page_start");
  await attr(() => db.createIntegerAttribute(DATABASE_ID, id, "source_page_end", false), "source_page_end");
  await attr(() => db.createBooleanAttribute(DATABASE_ID, id, "is_published", true), "is_published");
  await attr(() => db.createStringAttribute(DATABASE_ID, id, "created_by", 64, false), "created_by");
  await idx(id, "man_art_section_idx", ["section_id"]);
  await idx(id, "man_art_subsection_idx", ["subsection_id"]);
  await idx(id, "man_art_order_idx", ["order"]);
  await idx(id, "man_art_pub_order_idx", ["is_published", "order"], ["ASC", "ASC"]);
  return id;
}

async function main() {
  console.log("=== Appwrite Maneuvers Setup ===");
  console.log(`Database: ${DATABASE_ID}\n`);
  const sectionsId = await setupSections();
  const subsectionsId = await setupSubsections();
  const articlesId = await setupArticles();
  console.log("\n[4/4] maneuver-media bucket...");
  const bucket = await ensureBucket("maneuver-media");

  console.log("\n=== Setup Complete ===");
  console.log("Add these to your .env.local:\n");
  console.log(`VITE_APPWRITE_MANEUVERS_SECTIONS_COL_ID=${sectionsId}`);
  console.log(`VITE_APPWRITE_MANEUVERS_SUBSECTIONS_COL_ID=${subsectionsId}`);
  console.log(`VITE_APPWRITE_MANEUVERS_ARTICLES_COL_ID=${articlesId}`);
  console.log(`VITE_APPWRITE_MANEUVERS_MEDIA_BUCKET_ID=${bucket.$id}`);
}
