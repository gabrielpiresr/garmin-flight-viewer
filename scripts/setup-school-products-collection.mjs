...
const COLLECTION_ID = process.env.APPWRITE_SCHOOL_PRODUCTS_COL_ID || "school_products";
...
async function ensureCollection() {
  ...
  const col = await db.createCollection(DATABASE_ID, COLLECTION_ID, "school_products", PERMISSIONS, true, true);
  console.log(`  + Created collection school_products (${col.$id})`);
  return col;
}
...
async function main() {
  console.log("=== Appwrite School Products Collection Setup ===");
  await ensureCollection();
  await attr(() => db.createStringAttribute(DATABASE_ID, COLLECTION_ID, "school_id", 64, true), "school_id");
  await attr(() => db.createStringAttribute(DATABASE_ID, COLLECTION_ID, "name", 255, true), "name");
  await attr(() => db.createFloatAttribute(DATABASE_ID, COLLECTION_ID, "ideal_price", false, 0), "ideal_price");
  await attr(() => db.createBooleanAttribute(DATABASE_ID, COLLECTION_ID, "active", false, true), "active");
  await idx("products_school_idx", ["school_id"]);
  console.log("\nAdd to .env.local:");
  console.log(`VITE_APPWRITE_SCHOOL_PRODUCTS_COL_ID=${COLLECTION_ID}`);
  console.log("\nDone.");
}
