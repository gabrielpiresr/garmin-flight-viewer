 ...
 * src/flightRadarWatchCron.js and schedule every 3 minutes.
 *
 * Usage:
...
const schedule = process.env.FLIGHT_RADAR_WATCH_SCHEDULE || "*/3 * * * *";
const timeout = Number(process.env.FLIGHT_RADAR_WATCH_TIMEOUT || 90);
...
async function ensureFunction(functions) {
  const params = {
    ...
    enabled: true,
    logging: true,
    entrypoint: "src/flightRadarWatchCron.js",
...
async function ensureArchive(env) {
  if (fs.existsSync(archivePath) && process.env.FLIGHT_RADAR_WATCH_REUSE_ARCHIVE === "1") {
...
async function copyVariablesFromAdminUsers(functions, fileEnv, apiKey) {
  ...
  for (const variable of sourceVars) {
    ...
    if ((value == null || value === "") && variable.key === "FLIGHTRADAR24_API_TOKEN") {
      value = process.env.FLIGHTRADAR24_API_TOKEN || fileEnv.FLIGHTRADAR24_API_TOKEN || "";
    }
  ...
  const fr24Token =
    process.env.FLIGHTRADAR24_API_TOKEN || fileEnv.FLIGHTRADAR24_API_TOKEN || "";
  if (fr24Token) {
    await upsertVariable(functions, "FLIGHTRADAR24_API_TOKEN", fr24Token, true);
  }
...
async function main() {
  ...
  const code = InputFile.fromBuffer(buffer, "flight-radar-watch.tar.gz");
